-- ==================================================================
-- 00015: Server-side money integrity for offline replay + point adjustments
--   1. complete_sale_v1 no longer trusts client-sent sold_* totals: totals are
--      always recomputed from current menu prices. Offline replays still use
--      sold_at for the business date and are still created as 'completed'
--      (skip KDS), but the recorded money is computed by the server.
--      sold_* keys are still accepted in the payload for backward compatibility
--      with queued sales, but ignored.
--   2. sold_at may not be in the future (beyond 5 min clock skew).
--   3. Checkout status history now records the truth: 'new'->'new' for online
--      sales (creation event), 'new'->'completed' only for offline replays.
--   4. New adjust_customer_points_v1: locks the customer row, clamps at zero,
--      and logs the ACTUAL applied delta (not the requested one).
-- ==================================================================
...
BEGIN
  ...
  v_sold_at := (p_payload->>'sold_at')::timestamptz;
  IF v_sold_at IS NOT NULL AND v_sold_at > now() + interval '5 minutes' THEN
    RAISE EXCEPTION 'sold_at cannot be in the future';
  END IF;
  v_biz_date := get_business_date(COALESCE(v_sold_at, now()));
  ...
  v_initial_status := CASE WHEN v_offline_sync THEN 'completed' ELSE 'new' END;
  ...
  VALUES (v_order_number, v_seq, p_actor_user_id, v_customer_id, v_initial_status, 'paid', p_payload->>'payment_method', 0, 0, 0, 0, 0, v_biz_date, v_discount_type)
  ...
  -- Totals are always computed server-side. Client-sent sold_* totals from the
  -- offline queue are intentionally NOT honored: the server is the source of
  -- truth for money, and this closes the offline_sync total-spoofing hole.
  ...
  IF p_payload->>'payment_method' != 'cash' AND COALESCE(p_payload->>'gcash_reference', '') = '' THEN
    RAISE EXCEPTION 'Reference number is required for %', p_payload->>'payment_method';
  END IF;

  -- Cash must be fully tendered on the online path. Offline replays collected
  -- cash against the device's own total, so tendered_amount is informational.
  ...
  INSERT INTO order_status_history (order_id, from_status, to_status, changed_by_user_id) VALUES (v_order_id, 'new', v_initial_status, p_actor_user_id);
...
-- ==================================================================
-- Atomic, audited loyalty point adjustment.
-- Logs the delta that was ACTUALLY applied (after the zero clamp), so
-- balance_after is always reachable from balance_before + points_delta.
-- ==================================================================
CREATE OR REPLACE FUNCTION adjust_customer_points_v1(
  p_customer_id uuid,
  p_delta numeric,
  p_reason text,
  p_actor_user_id uuid
...
DECLARE
  v_customer record;
  v_new_balance numeric(10,0);
  v_actual_delta numeric(10,0);
BEGIN
  IF p_delta IS NULL OR p_delta = 0 THEN
    RAISE EXCEPTION 'Point delta must be non-zero';
  END IF;

  SELECT * INTO v_customer FROM customers WHERE id = p_customer_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Customer not found'; END IF;

  v_new_balance := GREATEST(0, v_customer.loyalty_points_balance + p_delta);
  v_actual_delta := v_new_balance - v_customer.loyalty_points_balance;

  UPDATE customers SET loyalty_points_balance = v_new_balance, updated_at = now() WHERE id = p_customer_id;

  INSERT INTO loyalty_transactions (customer_id, transaction_type, points_delta, balance_after, reason, actor_user_id)
  VALUES (p_customer_id, 'adjust', v_actual_delta, v_new_balance, NULLIF(p_reason, ''), p_actor_user_id);

  RETURN jsonb_build_object('balance', v_new_balance, 'actual_delta', v_actual_delta);
END;
$$;
