-- ==================================================================
-- 00009: Fix addon loop type - record -> jsonb \(regression from 00008\) (Senior/PWD 20%, Employee 10%)
--   - orders.discount_type column (senior_pwd | employee)
--   - discount computed on (subtotal + tax), grand = subtotal + tax - discount
--   - offline replay honors sold_discount_total
-- Online path without discount_type is byte-identical to 00007.
-- ==================================================================

ALTER TABLE orders ADD COLUMN IF NOT EXISTS discount_type text CHECK (discount_type IN ('senior_pwd','employee'));
...
BEGIN
  ...
  -- Discount on the total price (subtotal + tax): senior/pwd 20%, employee 10%
