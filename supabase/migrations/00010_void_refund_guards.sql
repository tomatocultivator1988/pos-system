-- 00010: Tighten void/refund guards — make them mutually exclusive and handle edge cases
-- Void: only new/preparing + payment must be paid (not already voided/refunded)
-- Refund: only ready/completed + payment must be paid (not voided/refunded), voided orders cannot be refunded
-- Keeps stock/money reversal logic identical to 00005/00009, just adds guards.
...
BEGIN
  ...
  IF v_order.status NOT IN ('ready', 'completed') THEN RAISE EXCEPTION 'Can only refund ready/completed orders, current status is %', v_order.status; END IF;
  IF v_order.payment_status IN ('voided', 'refunded') THEN RAISE EXCEPTION 'Order payment already %', v_order.payment_status; END IF;

  SELECT * INTO v_payment FROM payments WHERE order_id = p_order_id FOR UPDATE;
  IF v_payment.id IS NULL THEN RAISE EXCEPTION 'No payment found'; END IF;
  IF v_payment.status IN ('voided', 'refunded') THEN RAISE EXCEPTION 'Payment already %', v_payment.status; END IF;
  IF v_payment.status != 'paid' THEN RAISE EXCEPTION 'Payment not in paid status'; END IF;
