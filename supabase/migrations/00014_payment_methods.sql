-- 00014: Add BPI Bank Transfer + UnionBank Bank Transfer payment methods
-- Widen CHECK constraints on orders.payment_method and payments.method.
-- complete_sale_v1 (copy of 00012): require a reference number for any
-- non-cash method (was gcash-only).

ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_payment_method_check;
ALTER TABLE orders ADD CONSTRAINT orders_payment_method_check
  CHECK (payment_method IN ('cash','gcash','bpi_bank_transfer','unionbank_bank_transfer'));

ALTER TABLE payments DROP CONSTRAINT IF EXISTS payments_method_check;
ALTER TABLE payments ADD CONSTRAINT payments_method_check
  CHECK (method IN ('cash','gcash','bpi_bank_transfer','unionbank_bank_transfer'));
