...
import { getSalesByCategoryReport } from '@/lib/actions/reports'

type Row = { id: string; category: string; categoryId: string | null; qty: number; sales: number; net: number; cost: number; profit: number; margin: number }
type Totals = { items_sold: number; sales: number; net: number; cost: number; profit: number }
type TopCategory = { name: string; sales: number } | null
...
export default function SalesByCategoryPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null)
    try {
      const r = await getSalesByCategoryReport(dateFrom, dateTo)
      setRows(r.rows as Row[])
      setTotals(r.totals)
      setTopCategory(r.topCategory)
    } catch (e: any) {
  ...
  const exportCSV = () => {
    const headers = ['Category', 'Qty', 'Gross Sales', 'Net Sales', 'Cost', 'Profit', 'Margin %']
    const csv = [headers.join(','), ...rows.map(r => [r.category, r.qty, r.sales, r.net, r.cost, r.profit, r.margin].map(csvEscape).join(','))].join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'sales-by-category.csv'; a.click(); URL.revokeObjectURL(a.href)
  }
    ...
    <AppLayout>
      <div className="p-4 sm:p-6 lg:p-8 max-w-5xl">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl lg:text-3xl font-semibold mb-1">Sales by Category</h1>
            <p className="text-muted-foreground text-sm">Sales volume, cost, and profit per category</p>
          </div>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <div className="bg-accent/10 border border-accent rounded-xl p-4">
                <p className="text-sm text-muted-foreground mb-1 flex items-center gap-1.5"><Trophy className="w-4 h-4 text-accent" /> Top Category</p>
                <p className="text-lg font-semibold truncate">{topCategory?.name || '—'}</p>
                <p className="text-sm text-accent font-medium">{topCategory ? money(topCategory.sales) : ''}</p>
              </div>
                ...
                <tbody>
                  {rows.map(r => (
                    <tr key={r.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                      <td className="px-4 py-2.5 text-sm font-medium">{r.category}</td>
                  ...
                  {rows.length === 0 && (
                    <tr><td colSpan={7} className="px-4 py-12 text-center text-muted-foreground">No sales in this period</td></tr>
                  )}
                </tbody>
              </table>
            </div>

            <p className="text-xs text-muted-foreground mt-4">Gross = line_total + addons. Net = Gross − discount (pro-rata per item, same as receipt). Cost from recipes. Profit = Net − Cost. Excludes voided/refunded.</p>
          </>
        )}
      </div>
    </AppLayout>
